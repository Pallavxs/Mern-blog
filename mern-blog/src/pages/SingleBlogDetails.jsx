    ...
    return (
      <div className="md:flex-nowrap flex-wrap flex justify-between gap-20">
        {data && data.blog && (
          ...
          <div className='flex justify-between items-center'>
              <div className='flex justify-between items-center gap-5'>
                  <Avatar>
                      <AvatarImage src={data.blog.author?.avatar} className="rounded-full w-10 h-10 object-cover"/>
                  </Avatar>
                  <div>
                      <p className='font-bold'>{data.blog.author?.name}</p>
                      <p>Upload-Date : {moment(data.blog.createdAt).format('DD-MM-YYYY')}</p>
                  </div>
              </div>
              <div className='flex justify-between items-center gap-5'>
        ...
        <div className="border rounded md:w-[30%] w-full p-5">
          <RelatedBlog className='w-full' props={{ category: category, currentBlog: blog}}/>
        </div>
      </div>
    );
